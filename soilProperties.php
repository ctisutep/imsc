...
switch (true) {
  ...
  // case (isset($_POST['lat1']) && isset($_POST['lat2']) && isset($_POST['lng1']) && isset($_POST['lng2']) && isset($_POST['soilProperty']) && $_POST['soilProperty'] == 'aashtocl'):
  case (isset($_POST['district']) && isset($_POST['county']) && isset($_POST['soilProperty']) && $_POST['soilProperty'] == 'aashtocl'):
    ...
    // $query = "SET @geom1 = 'POLYGON(($lng1	$lat1,$lng1	$lat2,$lng2	$lat2,$lng2	$lat1,$lng1	$lat1))'";
    // $result = mysqli_query($conn, $query);
    // $query = "SELECT polygon.OGR_FID, ST_AsText(ST_Simplify(polygon.SHAPE, 0.0001)) AS POLYGON, polygon.areasymbol, chaashto.aashtocl AS classification, chaashto.mukey FROM polygon NATURAL JOIN chaashto WHERE ST_INTERSECTS(ST_GEOMFROMTEXT(@geom1, 1), SHAPE) AND chaashto.hzdept_r = 0";
    // $result = mysqli_query($conn, $query);
